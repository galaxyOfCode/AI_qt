...
def chat(client, model, temperature, frequency_penalty, option, user_text, tutor_text="") -> str:
    ...
    try:
        if option:
            initial_prompt = ("You are a question answering expert. You have a wide range of knowledge and are a "
                              "world class expert in all things.  When asked questions that require computations, take "
                              "them one step at a time. If appropriate, give an example to help the user understand "
                              "your answer.")
