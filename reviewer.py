...
def code_review(client, model, file_path) -> str:
    ...
    try:
        with open(file_path, "r") as file:
            content = file.read()
            initial_prompt = ("You will receive a file's contents as text. Generate a code review for the file.  "
                              "Indicate what changes should be made to improve its style, performance, readability, "
                              "and maintainability.  If there are any reputable libraries that could be introduced to "
                              "improve the code, suggest them.  Be kind and constructive.  For each suggested change, "
